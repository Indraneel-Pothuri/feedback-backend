...
def classify_with_api(text, labels):
    if not HF_API_KEY:
        return None, 0.0 # Return None so we know to use fallback
    ...
    try:
        ...
        if 'labels' in result and len(result['labels']) > 0:
            ...
            print("Model API Warning:", result)
            return None, 0.0
...
class Store(db.Model):
    ...
    def to_dict(self): return {"id": self.id, "name": self.name, "area": self.area}
...
def add_feedback():
    ...
    api_cat, api_score = classify_with_api(feedback_text, candidate_labels)
    
    if api_cat:
        category = api_cat # Use the AI's decision
    else:
        # B. Fallback to Rules ONLY if AI failed (e.g. network error)
        print("AI failed, using fallback rules.")
        text_lower = feedback_text.lower()
        if any(x in text_lower for x in ['food', 'tasty', 'burger']): category = "Quality of food"
        elif any(x in text_lower for x in ['service', 'waiter', 'rude']): category = "Customer service"
        elif any(x in text_lower for x in ['time', 'slow', 'fast']): category = "Speed"
        elif any(x in text_lower for x in ['ambience', 'music']): category = "Ambience"
...
def metrics():
    ...
    total = query.count()
    cat_query = query.group_by(Feedback.category).with_entities(
...
def reset_db():
    try:
        with app.app_context():
            ...
            db.session.commit()
        return jsonify({"message": "Database reset successfully!", "test_user": "admin / password123"}), 200
    except Exception as e:
